...
public class Upgrade
{
    ...
    public static final Upgrade ARTHROPOD = new Upgrade(asResource("arthropod"), Type.ARTIFACT, MobType.ARTHROPOD, DoomfulConfig::getArthropodDamageMultiplier, Upgrade::getArthropodName, getTargetName("arthropod"), DoomfulParticleTypes.ARTHROPOD);
    ...
    public static final Upgrade UNDEAD = new Upgrade(asResource("undead"), Type.RELIC, MobType.UNDEAD, DoomfulConfig::getUndeadDamageMultiplier, Upgrade::getUndeadName, getTargetName("undead"), DoomfulParticleTypes.UNDEAD);
    ...
    public static final Upgrade ILLAGER = new Upgrade(asResource("illager"), Type.RELIC, MobType.ILLAGER, DoomfulConfig::getIllagerDamageMultiplier, Upgrade::getIllagerName, getTargetName("illager"), DoomfulParticleTypes.ILLAGER);
    ...
    public static final Upgrade AQUATIC = new Upgrade(asResource("aquatic"), Type.RELIC, MobType.WATER, DoomfulConfig::getAquaticDamageMultiplier, Upgrade::getAquaticName, getTargetName("aquatic"), DoomfulParticleTypes.AQUATIC);
    ...
    public Upgrade(ResourceLocation key, Type type, MobType mobType, Supplier<Float> multiplierGetter, Supplier<Component> nameGetter, Component targetName, Supplier<SimpleParticleType> particleTypeGetter)
    {
        this(key, type, mob -> mob.getMobType() == mobType, multiplierGetter, nameGetter, targetName, particleTypeGetter);
    }
     ...
     */
    public static Optional<Upgrade> ofItem(ItemStack stack)
    {
        if (!stack.hasTag()) return Optional.empty();
        if (!stack.getOrCreateTag().contains(NBT_KEY)) return Optional.empty();
        ...
        @Nullable Upgrade upgrade = KEY_MAP.getOrDefault(key, null);
        return Optional.ofNullable(upgrade);
    }

    public static void clearUpgrade(ItemStack stack)
    {
        stack.getOrCreateTag().remove(NBT_KEY);
        if (stack.getOrCreateTag().isEmpty()) stack.setTag(null);
    }
    ...
    public void applyTo(ItemStack stack) { stack.getOrCreateTag().putString(NBT_KEY, key.toString()); }
